...
import { AlertCircle, CalendarIcon, Trash, Wrench, XIcon, Zap } from "lucide-react";
...
const priorityTexts = {
    LOW: { background: "bg-gray-100 dark:bg-gray-900", prioritycolor: "text-gray-600 dark:text-gray-400" },
    ...
    CRITICAL: { background: "bg-red-100 dark:bg-red-950", prioritycolor: "text-red-600 dark:text-red-400" },
};
                ...
                {["status", "type", "priority", "assignee"].map((name) => {
                    const options = {
                        ...
                        type: [
                            ...
                            { label: "Production Issue", value: "PRODUCTION_ISSUE" },
                        ],
                        priority: [
                                            ...
                                            <tr key={task.id} onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                                                ...
                                                <td className="px-4 py-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className={`uppercase text-xs ${color}`}>{task.type.replace("_", " ")}</span>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-2">
                                ...
                                return (
                                    <div key={task.id} className=" dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2">
                                        ...
                                        <div className="text-xs text-zinc-600 dark:text-zinc-400 flex items-center gap-2">
                                            {Icon && <Icon className={`size-4 ${color}`} />}
                                            <span className={`${color} uppercase`}>{task.type.replace("_", " ")}</span>
                                        </div>
