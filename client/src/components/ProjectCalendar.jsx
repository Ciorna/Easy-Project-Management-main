...
const typeColors = {
    PRODUCTION_ISSUE: "bg-red-200 text-red-800 dark:bg-red-500 dark:text-red-900",
    FEATURE: "bg-blue-200 text-blue-800 dark:bg-blue-500 dark:text-blue-900",
    MAINTENANCE: "bg-zinc-200 text-zinc-800 dark:bg-zinc-500 dark:text-zinc-900",
};
...
const priorityBorders = {
    ...
    CRITICAL: "border-red-600 dark:border-red-500 shadow-[0_0_10px_rgba(220,38,38,0.15)]",
};
...
const ProjectCalendar = ({ tasks }) => {
    ...
    const upcomingTasks = tasks
        .filter((task) => task.due_date && !isBefore(task.due_date, today) && task.status !== "COMPLETED")
        .sort((a, b) => new Date(a.due_date) - new Date(b.due_date))
        .slice(0, 5);

    const overdueTasks = tasks.filter((task) => task.due_date && isBefore(task.due_date, today) && task.status !== "COMPLETED");
                ...
                <div className="not-dark:bg-white dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        {daysInMonth.map((day) => {
                            ...
                            const hasOverdue = dayTasks.some((t) => t.status !== "COMPLETED" && isBefore(t.due_date, today));
